...
subprocess.run(["python", "frv_limit_hardrule.py"]) #Проверяем правило "Жесткий лимит ФРВ" при превышении нельзя утвердть график
subprocess.run(["python", "frv_limit_easyrule.py"]) #Проверяем правило "Мягкий лимит ФРВ" при превышении можно утвердить, но появляеться сообщение о превышении

#на третьем подписчике andrew4
subprocess.run(["python", "shiftexchrequest_add_without_position.py"]) #проверяем что заявка не создается если нет позиции в штатном расписании.
subprocess.run(["python", "shiftexchrequest_add_with_position.py"]) #проверяем что заявка  создается так как есть позиция в штатном расписании.