#проверка что заяка создается, если если мы добавили должность в штатном расписании и включена соответсвующая опция на подписчике
...
try:
    ...
    #СОЗДАЕМ ШТАТНОЕ РАСПИСАНИЕ
    
    #Нажимаем на шатаное расписание
    browser.find_element(By.CLASS_NAME, "icon-list").click()
    #Нажимаем на Заявки на биржу
    browser.find_element(By.LINK_TEXT, "Штатное расписание").click()
    #Нажимаем на кнопку добавить штатное расписание
    ...
    browser.find_element(By.CLASS_NAME, "select2-results__option.select2-results__option--highlighted").click()
    #Добавялем количество всего
    browser.find_element(By.ID, "id_required").clear()
    browser.find_element(By.ID, "id_required").send_keys("1")
    #нажимаем сохранить
    ...
    funciones.agregar_archivo("test.txt", "\n1 Заявка на биржу  создана так как есть должности")
