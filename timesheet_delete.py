...
try:
    ...
    #нажимаем на одину из записей табеля
    browser.find_element(By.XPATH, "//a[contains(@href, '/admin/worktime/timesheet/') and contains(@href, '/change')]").click()

    #переходим на новое окно
    browser.switch_to.window(browser.window_handles[1])

    #нажимаем на кнопку удалить запись табеля
    ...
    funciones.agregar_archivo("test.txt", "\n0 Удаление записи в табеле не произошло")
    funciones.capture_screenshot()
else:
    funciones.agregar_archivo("test.txt", "\n1 Удаление записи в табеле произошло")