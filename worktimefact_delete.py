#Удаление фактического отработанного времени
...
try:
    ...
    #нажимаем на одно из факстически отработанных времен
    browser.find_element(By.XPATH, "//a[contains(@href, '/admin/worktime/worktimefact/') and contains(@href, '/change')]").click()

    #переходим на новое окно
    browser.switch_to.window(browser.window_handles[1])

    #нажимаем на кнопку удалить факстически отработанное время
    ...
    funciones.agregar_archivo("test.txt", "\n0 Фактически отработанное время не удалилось")
    funciones.capture_screenshot()
else:
    funciones.agregar_archivo("test.txt", "\n1 Фактически отработанное время удалилось")